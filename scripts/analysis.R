...
indomain_eval <- subset(indomain_eval, !(Child %in% c('Emma', 'Violet', 'Naima')))
...
# training on all parent data then applied to each individual child, By Age

indomain_eval_parents <- read.csv('results/childes_indomain_parents_eval.txt', header = T, sep = '\t', row.names = NULL)

#indomain_eval_parents <- subset(indomain_eval_parents, !(Child %in% c('Emma', 'Violet', 'Naima')))
indomain_eval_parents$Child <- factor(indomain_eval_parents$Child, levels = c('Abe', 'Adam', 'Sarah', 'Thomas', 'Emma', 'Roman', 'Laura', 'Violet', 'Naima', 'Lily'))

indomain_eval_parents %>% 
  ggplot(aes(Age, as.numeric(Micro_LAS), group = Child, color = Child)) +
#  geom_line(aes(linetype = Child), alpha = 1) +
  #  scale_linetype_manual(values=c("dotted", 'solid', 'dotted', 'solid')) +
#  geom_point(aes(color = Child, shape = Child), size = 1.5) +
  geom_smooth(mapping = aes(Age, as.numeric(Micro_LAS)), method = 'gam') +
  #  scale_shape_manual(values = c(3, 16, 3, 16)) +
  #  scale_color_manual(values = c("darkred", "darkgreen", "steelblue",  "mediumpurple4")) +
  # scale_y_continuous(breaks=seq(50, 90, 10)) +
  facet_wrap( ~ Role) + #, scales = "free_y") +
...
ul_indomain_eval <- subset(ul_indomain_eval, !(Child %in% c('Emma', 'Violet', 'Naima')))
ul_indomain_eval$Child <- factor(ul_indomain_eval$Child, levels = c('Abe', 'Adam', 'Sarah', 'Thomas', 'Roman', 'Laura', 'Lily'))
...
#  geom_line(aes(linetype = Child), alpha = 1) +
  #  scale_linetype_manual(values=c("dotted", 'solid', 'dotted', 'solid')) +
#  geom_point(aes(color = Child, shape = Child), size = 1.5) +
  geom_smooth(mapping = aes(Sent_len, as.numeric(Micro_LAS)), method = 'gam') +
  #  scale_shape_manual(values = c(3, 16, 3, 16)) +
  #  scale_color_manual(values = c("darkred", "darkgreen", "steelblue",  "mediumpurple4")) +
  # scale_y_continuous(breaks=seq(50, 90, 10)) +
  facet_wrap( ~ Role) + #, scales = "free_y") +
...
# training on all parent data then applied to each individual child, By Utterance Length

ul_indomain_eval_parents <- read.csv('results/childes_indomain_parents_eval_ul.txt', header = T, sep = '\t', row.names = NULL)

ul_indomain_eval_parents <- subset(ul_indomain_eval_parents, !(Child %in% c('Emma', 'Violet', 'Naima')))
ul_indomain_eval_parents$Child <- factor(ul_indomain_eval_parents$Child, levels = c('Abe', 'Adam', 'Sarah', 'Thomas', 'Roman', 'Laura', 'Lily'))

ul_indomain_eval_parents %>% 
  ggplot(aes(Sent_len, as.numeric(Micro_LAS), group = Child, color = Child)) +
  #  geom_line(aes(linetype = Child), alpha = 1) +
  #  scale_linetype_manual(values=c("dotted", 'solid', 'dotted', 'solid')) +
  #  geom_point(aes(color = Child, shape = Child), size = 1.5) +
...
### More continuous plotting using results from regression file
...
regression$Train_size <- scale(regression$Train_size)
regression$Size_ratio1 <- scale(regression$Size_ratio1)
regression$UAS1 <- as.numeric(regression$UAS1)
regression$LAS1 <- as.numeric(regression$LAS1)
regression$UAS2 <- as.numeric(regression$UAS2)
regression$LAS2 <- as.numeric(regression$LAS2)

regression %>% 
  ggplot(aes(as.numeric(Age), as.numeric(LAS1), group = Child, color = Child)) +
  #  geom_line(aes(linetype = Child), alpha = 1) +
  #  scale_linetype_manual(values=c("dotted", 'solid', 'dotted', 'solid')) +
  #  geom_point(aes(color = Child, shape = Child), size = 1.5) +
  geom_smooth(mapping = aes(as.numeric(Age), as.numeric(LAS1)), method = 'gam') +
  #  scale_shape_manual(values = c(3, 16, 3, 16)) +
  #  scale_color_manual(values = c("darkred", "darkgreen", "steelblue",  "mediumpurple4")) +
...
subset(regression, Role == 'child') %>% 
  ggplot(aes(as.numeric(Age), as.numeric(LAS2), group = Child, color = Child)) +
  #  geom_line(aes(linetype = Child), alpha = 1) +
  #  scale_linetype_manual(values=c("dotted", 'solid', 'dotted', 'solid')) +
  #  geom_point(aes(color = Child, shape = Child), size = 1.5) +
  geom_smooth(mapping = aes(as.numeric(Age), as.numeric(LAS2)), method = 'gam') +
  #  scale_shape_manual(values = c(3, 16, 3, 16)) +
  #  scale_color_manual(values = c("darkred", "darkgreen", "steelblue",  "mediumpurple4")) +
  scale_x_continuous(breaks=seq(0, 72, 6)) +
  facet_wrap( ~ Role) + #, scales = "free_y") +
...
regression %>% 
  ggplot(aes(as.numeric(Sent_len), as.numeric(LAS1), group = Child, color = Child)) +
  #  geom_line(aes(linetype = Child), alpha = 1) +
  #  scale_linetype_manual(values=c("dotted", 'solid', 'dotted', 'solid')) +
  #  geom_point(aes(color = Child, shape = Child), size = 1.5) +
  geom_smooth(mapping = aes(as.numeric(Sent_len), as.numeric(LAS1)), method = 'gam') +
  facet_wrap( ~ Role) + #, scales = "free_y") +
  theme_classic() + 
  theme(legend.position="top") +
  theme(text = element_text(size=22, family="Times"),
...
subset(regression, Role == 'child') %>% 
  ggplot(aes(as.numeric(Sent_len), as.numeric(LAS2), group = Child, color = Child)) +
  #  geom_line(aes(linetype = Child), alpha = 1) +
  #  scale_linetype_manual(values=c("dotted", 'solid', 'dotted', 'solid')) +
  #  geom_point(aes(color = Child, shape = Child), size = 1.5) +
  geom_smooth(mapping = aes(as.numeric(Sent_len), as.numeric(LAS2)), method = 'gam') +
  facet_wrap( ~ Role) + #, scales = "free_y") +
  theme_classic() + 
  theme(legend.position="top") +
  theme(text = element_text(size=25, family="Times")) +
...
### Comparing scores for each child based on different parsers 
### trained on all child-parent interactions from other children vs. trained on all parent data

for (child in unique(regression$Child)){
  print(child)
  m <- lm(LAS2 ~ LAS1, data = subset(regression, Role == 'child' & Child == child))
  print(summary(m)$coef)
}

############## Regression analysis: Overall #######

child_regression <- subset(regression, Role == 'child')
child_regression$total_DL <- (child_regression$Sent_len + 1) * child_regression$DL

model <- lmer(LAS1 ~ Train_size *  Age_int * Sent_len *total_DL + (Train_size | Child), data = child_regression)
