	...
	for file in os.listdir('../processed_data/' + child + '/'):
		for seed in [1, 2, 3]:
			if file + '.machamp.twitter.' + str(seed) + '.indomain' not in os.listdir('../predict/machamp/' + child + '_indomain/') or os.stat('../predict/machamp/' + child + '_indomain/' + file + '.machamp.twitter.' + str(seed) + '.cardiffnlp-twitter-roberta-base.indomain').st_size == 0:
				...
				#	try:
				os.system("python3 predict.py logs/" + child + ".machamp." + str(seed) + '.cardiffnlp-twitter-roberta-base/*/model.tar.gz ../processed_data/' + child + '/' + file + ' ../predict/machamp/' + child + '_indomain/' + file + '.machamp.twitter.' + str(seed) + '.cardiffnlp-twitter-roberta-base.indomain --device 0 --batch_size 16')
				#	except:
