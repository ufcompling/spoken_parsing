...
f = open('results/' + 'childes_indomain_parents_eval.txt', 'w')
f.write('\t'.join(w for w in header) + '\n')

ul_header = ['Child', 'Corpus', 'Role', 'Sent_len', 'Micro_LAS']
		
ul_f = open('results/' + 'childes_indomain_parents_eval_ul.txt', 'w')
ul_f.write('\t'.join(w for w in ul_header) + '\n')

for directory in ['Abe_Kuczaj', 'Adam_Brown', 'Emma_Weist', 'Laura_Braunwald', 'Lily_Providence', 'Naima_Providence', 'Roman_Weist', 'Sarah_Brown', 'Thomas_Thomas', 'Violet_Providence']:
	data_seed1 = []
	data_seed2 = []
	data_seed3 = []	
	ul_evaluate_dict = {}
	for gold_file in os.listdir('processed_data/' + directory):
		if gold_file.endswith('.child'):
			...
			#	try:
				if 2 > 1:
					pred_file = 'predict/machamp/' + directory + '_indomain_parents/' + gold_file + '.machamp.twitter.' + str(seed) + '.cardiffnlp-twitter-roberta-base.indomain'
