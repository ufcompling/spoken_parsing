...
header = ['Child', 'Corpus', 'Role', 'Train_size', 'Parent_Train_size', 'Test_size', 'Size_ratio1', 'Size_ratio2', 'Age', 'Sent_len', 'DL', 'UAS1', 'LAS1', 'UAS2', 'LAS2']
f = open('results/childes_indomain_regression.txt', 'w')
f.write('\t'.join(w for w in header) + '\n')

dl_header = ['Child', 'Corpus', 'Role', 'Age', 'DL', 'DL_Direction', 'UAS1', 'LAS1', 'UAS2', 'LAS2', 'Gold_Deprel1', 'Pred_Deprel1', 'Gold_Deprel2', 'Pred_Deprel2']
		
dl_f = open('results/' + 'childes_indomain_regression_dl.txt', 'w')
dl_f.write('\t'.join(w for w in dl_header) + '\n')

parent_train_data = get_parse('processed_data/' + directory + '/train_parents.conllu')
parent_train_size = 0
for parse in parent_train_data:
	parent_train_size += len(parse)
...
#			micro_las = []
			...
			for seed in [1]: #, 2, 3]:
				...
				try:
					parent_pred_file = 'predict/machamp/' + directory + '_indomain_parents/' + gold_file + '.machamp.twitter.' + str(seed) + '.cardiffnlp-twitter-roberta-base.indomain'
				
					parent_evaluate_info, parent_test_set_size = evaluate('processed_data/' + directory + '/' + gold_file, parent_pred_file)
				except:
					pass

				for i in range(len(evaluate_info)):
					info = evaluate_info[i]
					new_tok = ''
					try:
						parent_info = parent_evaluate_info[i]
						info += parent_info[-2 : ]
						new_tok = [child, corpus, role, train_size, parent_train_size, test_set_size, train_size / test_set_size, parent_train_size / test_set_size]
					except:
						info += ['', '']
						new_tok = [child, corpus, role, train_size, parent_train_size, test_set_size, train_size / test_set_size, '']
					new_tok += info
					f.write('\t'.join(str(w) for w in new_tok) + '\n')

				dl_evaluate_info = dl_evaluate('processed_data/' + directory + '/' + gold_file, pred_file)
				try:
					parent_dl_evaluate_info = dl_evaluate('processed_data/' + directory + '/' + gold_file, parent_pred_file)
				except:
					pass

				for i in range(len(dl_evaluate_info)):
					info = dl_evaluate_info[i]
					try:
						parent_info = parent_dl_evaluate_info[i]
						info += parent_info[-4 : ]
					except:
						info += ['', '', '', '']
					new_tok = [child, corpus, role] + info
