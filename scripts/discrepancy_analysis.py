...
#		for emb in ['cardiffnlp-twitter-roberta-base']:
		for emb in ['bert-base-cased', 'roberta-base', 'cardiffnlp-twitter-roberta-base']:
			...
			#	for age_range in ['48_54']: #['18_24', '24_30']: #, '30_36', '36_42', '42_48', '48_54', '54_60', '60_66']:
				...
				#		if 'nmod:poss' in discrepancies_proportion:
				#			nmod.append(float(discrepancies_proportion['nmod:poss']))
				#			nmod_specific.append(float(details_proportion['nmod:poss']['nmod']))
				#		if 'discourse' in discrepancies_proportion or 'vocative' in discrepancies_proportion:
				#			try:
				#				nmod.append(float(discrepancies_proportion['discourse']) + float(discrepancies_proportion['vocative']))
				#				nmod_specific.append(float(details_proportion['discourse']['root']) + float(details_proportion['vocative']['root']))
				#			except:
				#				print('nah')
				#				pass
						if 'vocative' in discrepancies_proportion:
							nmod.append(float(discrepancies_proportion['vocative']))
							try:
								nmod_specific.append(float(details_proportion['vocative']['nsubj']))
							except:
								pass
						#	try:
						#		nmod_specific.append(float(details_proportion['vocative']['nsubj']) + float(details_proportion['vocative']['obj']))
						#	except:
						#		try:
						#			nmod_specific.append(float(details_proportion['vocative']['nsubj']))
						#		except:
						#			nmod_specific.append(float(details_proportion['vocative']['obj']))

				#		if 'nsubj' in discrepancies_proportion:
				#			nmod.append(float(discrepancies_proportion['nsubj']))
				#			try:
				#				nmod_specific.append(float(details_proportion['conj']['compound']))
				#			except:
				#				pass

print(statistics.mean(nmod))
print(statistics.mean(nmod_specific))
